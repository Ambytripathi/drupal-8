...
class ProfileEditorForm extends FormBase {

  public function getFormId() {
    
    return 'profile_editor_form';
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $query_result->condition(
    		db_or()
    		->condition('c.role', 'diplomats')
    		->condition('c.role','partners')
    		);
    ...
    foreach($data as $row){
      ...
      if($row->refer_management_approval== 0){
        ...
        $status = 'Pending';
        //$action = '<div id="'.$row->id.'" class="refer-activation-editor btn btn-success editor_activate ">Activate Refer & Earn</div>';
        //$rejection = '<div id="'.$row->id.'" class="refer-reject-editor btn btn-danger editor_reject">Reject</div>';

      } else if($row->refer_management_approval== 1){

        $status = 'Activated';
        //$action = '<div id="'.$row->id.'" class="refer-deactivation-editor btn btn-danger editor_deactivate ">De-Activate Refer & Earn</div>';
      ...
      if($row->refer_editor_approval== 0){
      	
      	//$status = 'Pending';
      	$action = '<div id="'.$row->id.'" class="refer-activation-editor btn btn-success editor_activate ">Activate Refer & Earn</div>';
      	$rejection = '<div id="'.$row->id.'" class="refer-reject-editor btn btn-danger editor_reject">Reject</div>';
      	
      } else if($row->refer_editor_approval== 1){
      	
      	//$status = 'Activated';
      	$action = '<div id="'.$row->id.'" class="refer-deactivation-editor btn btn-danger editor_deactivate ">De-Activate Refer & Earn</div>';
      	$rejection = '<div id="'.$row->id.'" class="no-action btn btn-primary">No Action</div>';
      	
      } else if($row->refer_editor_approval== 2){
      ...
      else if($row->refer_editor_approval== 3){
    ...
    $form['reset'] = array(
      ...
      '#value' => $this->t('Reset'),
      '#button_type' => 'success',
      '#suffix' => '<div id="profile-informations-editor">'.$info_values.'</div>'
    );
  ...
  public function _offersfiltersubmit(array &$form, FormStateInterface $form_state) {
    ...
    if(count($data) > 0){
      ...
      foreach($data as $row){
       ...
       // else
        //{
       // 	$info_values .= '<tr><td>'.$row->id.'</td><td>'.$row->first_name.'</td><td>'.$row->last_name.'</td><td>'.$row->email.'</td><td>'.$roles.'</td><td>'.$editor_approval.'</td><td>'.$mgmt_approval.'</td><td>'.$status.'</td><td>'.$action.'</td><td>'.$rejection.'</td><td><a class="btn btn-primary" href="/user/'.$row->id.'/view">View Profile</a></td></tr>';
       // }
     ...
     $ajax_response->addCommand(new HtmlCommand('#profile-informations-editor', $info_values));
